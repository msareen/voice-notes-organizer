...
import { modal, pickList, checkbox } from "../widgets.ts";
import type { PickHandle, PickItem } from "../widgets.ts";

/** What GET /api/cleanup/scan reports back. */
interface ScanResult {
  threshold: number;
  scanned: number;
  short: { rel: string; name: string; durationSec: number }[];
  originals?: { rel: string; name: string; recordingName: string; size: number }[];
}

export function openCleanup(): void {
  var thresholdInput: HTMLInputElement;
  var originalsBox: HTMLInputElement;
  modal({
    ...
    onConfirm: function () {
      ...
      return api<ScanResult>(url).then(function (res) { showCleanupResults(res, wantOriginals); });
    }
  });
}
...
function showCleanupResults(res: ScanResult, wantOriginals: boolean): void {
  ...
  var originalRels: Record<string, boolean> = {};
  originals.forEach(function (o) { originalRels[o.rel] = true; });

  var items: PickItem[] = res.short.map(function (s): PickItem {
    return { value: s.rel, name: s.name, sub: s.rel, right: s.durationSec.toFixed(1) + "s" };
  }).concat(originals.map(function (o): PickItem {
    return {
      value: o.rel,
      name: o.name,
  ...
  modal({
    ...
    onConfirm: function () {
      ...
      return api<{ removed?: number; removedOriginals?: number }>("/api/cleanup", { method: "POST", body: body })
        .then(function (out) {
