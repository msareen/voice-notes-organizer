...
import type { ProgressReport, ScanProgress, WorkProgress } from "../types.ts";
...
export interface ProgressBar {
   ...
   */
  report(event: ProgressReport | null | undefined): void;
...
export function createProgressBar(label: string): ProgressBar {
  ...
  let lastDrawAt = 0;
  // last event, so a forced redraw has something to draw
  let current: ScanProgress | WorkProgress | null = null;
  ...
  function render(event: ScanProgress | WorkProgress): string {
   ...
   */
  function logLine(message: string, level?: LogLevel) {
    const text =
 ...
 */
export function locationOf(file: string, root: string): { dir: string; name: string } {
