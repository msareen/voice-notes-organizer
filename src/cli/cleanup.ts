...
/** A recording queued for deletion, with its duration when one was measured. */
interface Doomed {
  file: string;
...
export interface CleanupOptions {
  threshold?: number;
  dryRun?: boolean;
  /** Named recordings to delete instead of scanning by duration. */
  files?: string[] | null;
  /** Also offer the `*.original.m4a` pre-repair backups. */
  originals?: boolean;
}
 ...
 * `runCleanup` for why.
 */
async function deleteOriginalBackups(backups: string[], target: string): Promise<number> {
  let removed = 0;
  for (const file of backups) {
    ...
    try {
      ...
      console.log(chalk.red(`Failed to delete ${path.relative(target, file)}: ${errorMessage(err)}`));
    }
  }
  return removed;
}
 ...
 */
async function resolveNamed(names: string[], allAudio: string[], target: string): Promise<Doomed[] | null> {
  const seen = new Set<string>();
  const resolved: Doomed[] = [];
  let missing = 0;
...
async function findShortRecordings(
  allAudio: string[],
  threshold: number,
  target: string
): Promise<Doomed[] | null> {
  ...
  try {
    ...
    bar.report({ phase: "work", done: allAudio.length, total: allAudio.length, dir: "", name: "" });
  } finally {
    bar.stop();
  }
  return short;
...
async function existingTranscripts(audioPath: string): Promise<string[]> {
  const found: string[] = [];
...
async function deleteRecordings(
  files: string[],
  target: string
): Promise<{ removed: number; deleted: DeletionItem[] }> {
  let removed = 0;
 ...
 */
export async function runLedgerCleanup(): Promise<void> {
  const config = await loadConfig();
