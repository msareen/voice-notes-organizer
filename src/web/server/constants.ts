...
// app.ts split across assets/js/*.ts doesn't need a new entry per file.
//
// `.ts` is here because the browser modules ship as TypeScript and are
// transpiled per request (see assets.ts) - the response is JavaScript, so the
// content type has to say so.
