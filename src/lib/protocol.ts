 ...
 * `vno v` (via `vno open-protocol`, bin/vno.ts's handler for it).
 *
 ...
 *
 *  `process.execPath` is the Bun binary, and the entry point is the .ts source
 *  Bun runs directly, so this needs no build output to point at. */
export function protocolCommandLine(): string {
  const vnoEntry = fileURLToPath(new URL("../../bin/vno.ts", import.meta.url));
  return `"${process.execPath}" "${vnoEntry}" open-protocol "%1"`;
}

/** Reads the currently registered command line, or null if nothing's registered. */
async function readRegisteredCommand(): Promise<string | null> {
  ...
  //   (Default)    REG_SZ    "C:\...\bun.exe" "C:\...\vno.ts" open-protocol "%1"
 ...
 */
export async function protocolStatus(): Promise<ProtocolStatus> {
...
export async function registerProtocol(): Promise<ProtocolResult> {
...
export async function unregisterProtocol(): Promise<ProtocolResult> {
