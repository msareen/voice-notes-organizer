...
// filter box (list.ts) and the deck's transcript highlight (deck.ts).
//
...
interface IndexEntry {
  /** Display copy, original case, for snippets. */
  text: string;
  lower: string;
  /** Title + name + transcript, lowercased: one indexOf answers the search. */
  hay: string;
}

var index = new WeakMap<Note, IndexEntry>();

export function normalize(s: unknown): string {
  return String(s == null ? "" : s).replace(/\s+/g, " ").trim();
}

function entryFor(note: Note): IndexEntry {
 ...
 */
export function warmIndex(notes: Note[]): void {
  var i = 0;
  function slice(deadline: IdleDeadline | null) {
...
function schedule(fn: (deadline: IdleDeadline | null) => void): void {
...
export function matches(note: Note, term: string): boolean {
  return !term || entryFor(note).hay.indexOf(term) !== -1;
}

/** Transcript context around a hit, with ellipses where the text was cut. */
 ...
 */
export function snippet(note: Note, term: string, pad?: number): Snippet | null {
  if (!term) return null;
  var e = entryFor(note);
 ...
 */
export function markInto(el: HTMLElement, text: string, term: string): number {
  el.textContent = "";
