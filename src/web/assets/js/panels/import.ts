...
import { button, modal, pickList, checkbox } from "../widgets.ts";
import { missingDeps } from "../deps.ts";
import type { KnownMount } from "../../../../types.ts";
import type { PickHandle } from "../widgets.ts";

/** A row of GET /api/volumes - see server/routes/import.ts:listVolumes. */
...
/** One level of the folder browser, as GET /api/browse reports it. */
...
export function openImport(): void {
  ...
  api<{ volumes: ImportVolume[] }>("/api/volumes").then(function (res) {
    ...
    var rememberChk: HTMLInputElement;
    var subdirs: Record<string, string> = {};
...
function browseFolders(
  ...
  api<BrowseLevel>("/api/browse?volume=" + encodeURIComponent(volume.id) + "&sub=" + encodeURIComponent(sub || ""))
    .then(function (res) {
      var level = modal({
  ...
  function folderRow(label: string, onClick: () => void) {
