...
/** Callers pass this to stream repair progress into a job log. */
...
export function originalBackupFor(filePath: string): string {
...
export function isOriginalBackup(name: string): boolean {
 ...
 */
export async function listOriginalBackups(audioFiles: string[]): Promise<OriginalBackup[]> {
  const found: OriginalBackup[] = [];
...
/** One MP4 box: where its header, payload and declared end sit in the buffer. */
interface Atom {
  type: string;
  start: number;
  /** First byte of the payload. */
  body: number;
  /** One past the last byte the header claims, which may run past EOF. */
  end: number;
  truncated: boolean;
}

/** Walks atoms in [start, end), tolerating declared sizes that run past EOF. */
function walkAtoms(buf: Buffer, start: number, end: number): Atom[] {
  const out: Atom[] = [];
  let off = start;
  while (off + 8 <= end) {
...
function findAtom(buf: Buffer, atomPath: string): Atom | null {
  const parts = atomPath.split("/");
  let atoms = walkAtoms(buf, 0, buf.length);
  let hit: Atom | null = null;
...
export function isSamsungVoiceRecording(buf: Buffer): boolean {
 ...
 */
export async function isTruncatedMp4(filePath: string): Promise<boolean> {
  let handle = null;
  try {
 ...
 */
export async function healIfDamaged(filePath: string, { onOutput }: RepairOptions = {}): Promise<boolean> {
...
function extractASC(buf: Buffer, stsd: Atom): Buffer {
...
function parseASC(asc: Buffer): AudioSpecificConfig {
 ...
 */
function expectedFrameCount(buf: Buffer): number | null {
...
function log(message: string, onOutput: OutputCallback | null | undefined): void {
  if (onOutput) onOutput(message);
}

async function probeDurationOf(ffprobe: string, file: string): Promise<number | null> {
 ...
 */
export async function repairSamsungM4A(
  filePath: string,
  { onOutput }: RepairOptions = {}
): Promise<string | null> {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`ffmpeg couldn't re-frame the recovered audio: ${lastLine(execErrorText(err))}`);
    }
    ...
    try {
      ...
      throw new Error(`couldn't put the repaired audio in place: ${errorMessage(err)}`);
    }
    staged = null;
    ...
    log(`Samsung m4a repair skipped for ${name}: ${errorMessage(err)}`, onOutput);
    return null;
  } finally {
...
/**
 * execFile rejects with an Error carrying the child's stderr, which is where
 * ffmpeg's actual complaint lives - the message alone is just "Command failed".
 */
