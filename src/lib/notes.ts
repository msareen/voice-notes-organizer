...
export const TRANSCRIPT_EXTS: readonly TranscriptExt[] = [".vtt", ".srt", ".txt"];

const stripExt = (p: string) => p.slice(0, -path.extname(p).length);
...
export async function findTranscript(audioPath: string): Promise<FoundTranscript | null> {
...
/** What a `-f` name resolved to; see `resolveNamedFile`. */
export interface ResolvedName {
  /** null both when nothing matched and when the name was ambiguous. */
  file: string | null;
  /** The candidates, so a caller can tell those two cases apart. */
  matches: string[];
}
 ...
 */
export async function resolveNamedFile(
  name: string,
  allAudio: string[],
  target: string
  ...
  const baseOf = (f: string) => path.basename(f).toLowerCase();
  const stemOf = (f: string) => {
    const b = baseOf(f);
 ...
 */
export function reportUnresolved(name: string, result: ResolvedName, target: string): void {
...
export async function readTranscript(audioPath: string): Promise<TranscriptRead> {
...
interface BuildNoteOptions {
  size?: number | null;
  mtimeMs?: number | null;
  /** A number reuses a cached probe; `undefined` always probes fresh. */
...
async function buildNote(
  target: string,
  audioPath: string,
  { size, mtimeMs, durationSec }: BuildNoteOptions = {}
): Promise<Note> {
 ...
 */
export async function refreshNote(target: string, audioPath: string): Promise<Note> {
  let size: number | null = null;
  let mtimeMs: number | null = null;
  try {
    const stat = await fs.stat(audioPath);
