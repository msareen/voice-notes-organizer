 ...
 * server restart.
 */
export async function serveAsset(relPath: string): Promise<Response> {
  ...
  if (full !== ASSET_ROOT && !full.startsWith(ASSET_ROOT + path.sep)) {
    return Response.json({ error: "Path outside the assets folder" }, { status: 400 });
  }
  const ext = path.extname(full).toLowerCase();
  const type = ASSET_MIME[ext];
  if (!type) return Response.json({ error: `No asset at ${relPath}` }, { status: 404 });

  let body: Buffer;
  try {
    body = await fs.readFile(full);
  } catch {
    return Response.json({ error: `No asset at ${relPath}` }, { status: 404 });
  }
  ...
  if (ext === ".ts") {
    try {
      ...
      return Response.json({ error: `Couldn't compile ${relPath}: ${message}` }, { status: 500 });
    }
  }

  return new Response(payload, { headers: { "Content-Type": type, "Cache-Control": "no-store" } });
}
