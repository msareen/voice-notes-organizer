 ...
 *
 * Lives in lib/ rather than web/ for the same reason themes.ts does: both
...
export interface WhisperLanguage {
  /** whisper.cpp's own code (ISO-639-1, apart from `haw` and `yue`). */
  code: string;
  label: string;
}
...
export const LANGUAGE_CODES: readonly string[] = WHISPER_LANGUAGES.map((l) => l.code);
...
export function isLanguageChoice(value: unknown): value is string {
  return value === "auto" || (typeof value === "string" && LANGUAGE_CODES.includes(value));
}
 ...
 */
export function normalizeLanguageMap(raw: unknown): Record<string, string> {
