...
/**
 * One row of the picker. inquirer's Choice objects are shared between the
 * master collection and every filtered view, which is what makes checked state
 * survive filtering.
 */
interface Row {
  name: string;
...
/** The keypress payload inquirer's event stream emits. */
...
export default class SearchableCheckboxPrompt extends CheckboxPrompt {
  /** The full, unfiltered collection; `this.opt.choices` is a view onto it. */
  declare master: any;
  declare searchTerm: string;

  _run(cb: (value: unknown) => void): this {
    this.done = cb;
  ...
  applyFilter(): void {
    ...
    view.choices = master.choices.filter((choice: Row) => {
  ...
  onKeypress({ value, key: rawKey }: KeypressPayload): void {
    const key = rawKey || {};
    ...
    if (key.ctrl && key.name === "a") {
      const rows: Row[] = this.opt.choices.realChoices;
...
function renderChoices(rows: Row[], pointer: number): string {
  let out = "";
...
inquirer.registerPrompt("searchable-checkbox", SearchableCheckboxPrompt as never);
