...
async function walk(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // beside it (see special-case-handling.ts), not a separate recording -
 ...
 */
export function reporter(onProgress: ProgressCallback | null | undefined): Reporter {
...
export async function syncVolume(
  volume: SyncSource,
  target: string,
  { rememberDeletions = true, onProgress }: SyncOptions = {}
): Promise<SyncResult> {
  ...
  const isDeleted = (dest: string, size: number | null) =>
    wasDeleted(path.relative(target, dest).split(path.sep).join("/"), size);
  ...
  for (const src of files) {
    ...
    try {
      ...
      report({ phase: "log", level: "warn", message: `Failed to copy ${label}: ${errorMessage(err)}` });
    } finally {
      done++;
    }
  }
...
/** `catch` binds `unknown` under strict mode; this is the one place that matters here. */
function errorMessage(err: unknown): string {
  return err instanceof Error ? err.message : String(err);
}

/** Best-effort source removal after a successful import copy; failures are logged, not thrown. */
async function tryDeleteSource(src: string, label: string, report: Reporter): Promise<number> {
  try {
    await fs.remove(src);
    return 1;
  } catch (err) {
    report({ phase: "log", level: "warn", message: `Imported ${label} but couldn't delete it from the source: ${errorMessage(err)}` });
    return 0;
  }
}

export interface FlatDest {
  dest: string;
  skip: boolean;
  wasDeletedBefore: boolean;
}
 ...
 */
export async function resolveFlatDest(
  destRoot: string,
  base: string,
  srcSize: number | null,
  isDeleted: (dest: string, size: number | null) => boolean = () => false
): Promise<FlatDest> {
 ...
 */
export async function flattenVolumeFolder(folder: string): Promise<number> {
  let moved = 0;
  let entries;
  try {
...
async function moveFilesUp(dir: string, root: string): Promise<number> {
  let moved = 0;
  let entries;
  try {
    entries = await fs.readdir(dir, { withFileTypes: true });
 ...
 * `routes/import.ts:browse()` uses for the equivalent read-side picker.
 * Falls back to `target` itself if every segment gets filtered out.
 */
function resolveMappedDest(target: string, mapTo: string): string {
 ...
 */
export function sourceDestFolder(source: Pick<Source, "path" | "mapTo">, target: string): string {
