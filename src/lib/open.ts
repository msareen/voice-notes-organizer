...
function launch(cmd: string, args: string[]): boolean {
  try {
 ...
 */
export function revealInFolder(target: string): boolean {
...
// Matches the manifest's `name` (web/page.ts:renderManifest) - Chrome/Edge
 ...
 * (lib/protocol.ts), since neither Chrome's nor Edge's PWA install leaves an
 ...
 */
export async function findInstalledPwaShortcut(): Promise<string | null> {
...
async function findShortcutIn(dir: string, depth: number): Promise<string | null> {
  let entries;
  try {
