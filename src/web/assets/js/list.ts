// The left takes list, grouped by folder. Mutually referential with deck.ts
...
import { fmtDur, fmtSize } from "./format.ts";
import { select, highlightTranscript } from "./deck.ts";
import { revealFolder } from "./actions.ts";
import { normalize, matches, snippet, markInto, warmIndex } from "./search.ts";
import type { Note } from "../../../types.ts";

/**
 * A take's row element. The note, its rel and its group hang off the element
 * itself so the filter and keyboard navigation can read them straight back
 * off the DOM rather than keeping a parallel index in step with it.
 */
interface TakeRow extends HTMLDivElement {
  _rel: string;
  _note: Note;
  _group: Group;
  /** The transcript context line, created on the row's first search hit. */
  _hit?: HTMLDivElement;
}
...
var collapsed: Record<string, boolean> = {};
...
export function renderList(): void {
  ...
  rows = [];
  groups = [];
  var groupByKey: Record<string, Group> = {};

  var totalSec = 0;
  for (var n = 0; n < state.NOTES.length; n++) {
    if (state.NOTES[n].durationSec != null) totalSec += state.NOTES[n].durationSec!;
  }
  document.getElementById("statTakes")!.textContent = String(state.NOTES.length);
  ...
  state.NOTES.forEach(function (note) {
    var row = document.createElement("div") as TakeRow;
    row.className = "row";
    row.tabIndex = 0;
  ...
  groups.forEach(function (g) {
    if (showGroups) {
      ...
      gcount.textContent = String(g.rows.length);
...
function applyCollapsed(g: Group): void {
...
function rowVisible(r: TakeRow): boolean {
  return !r.classList.contains("hidden") &&
    !(r._group.body != null && r._group.body.classList.contains("collapsed"));
}
...
dom.list.addEventListener("keydown", function (e) {
  ...
  var pos = vis.indexOf(document.activeElement as TakeRow);
  if (pos === -1) {
...
/* ---- Filter box ----------------------------------------------------------
   ...
   text (js/search.ts). Transcript hits show the matching line under the row,
...
function showHit(row: TakeRow, term: string): void {
